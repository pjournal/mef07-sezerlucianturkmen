...
# 1. Calculate the mean of all streams in the dataset
mean_streams <- filtered_spotify_data %>%
  summarise(mean_streams = mean(streams, na.rm = TRUE))

mean_streams

# 2. Calculate the mean of streams for each month
mean_streams_by_month <- filtered_spotify_data %>%
  group_by(released_month) %>%
  summarise(mean_streams = mean(streams, na.rm = TRUE))
mean_streams_by_month

# 3. Transmute to create a new dataset with only selected columns
